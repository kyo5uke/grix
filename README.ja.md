...
| --------------------------------- | -----: | ------: | -----: | ----: |
| `PageTransHuge`（まれなリテラル）          |      5 |  2.78 s |  15 ms |  181× |
| `EXPORT_SYMBOL`（頻出リテラル）           | 38,267 |  2.41 s | 174 ms | 13.9× |
| `static\s+int\s+\w+_probe`（regex） | 10,081 |  2.40 s | 301 ms |  8.0× |
| `spinlock`（`-i`）                  | 17,086 |  2.41 s | 217 ms | 11.1× |
| `zzqqxx_does_not_exist`（ヒットなし）    |      0 |  2.36 s |  16 ms |  145× |

索引サイズは 126 MiB です（どのファイルにもあるような超高頻度トライグラムは
絞り込みに寄与しないため件数だけを保存。結果に影響なく約2割の削減）。
ゼロからの構築は、ファイルシステムキャッシュが温まった状態で約 10 秒です
（冷えている場合は I/O 律速でディスク次第です）。

無変更時の `grix index` は約 0.35 秒です。
並列のディレクトリ走査だけで、何も書き込みません。1ファイル変更した場合も
約 0.5 秒です — 変更分は小さなオーバーレイに入り、索引本体は触りません。
